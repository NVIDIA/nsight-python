...
- Calling :func:`@nsight.analyze.kernel <nsight.analyze.kernel>`-decorated functions from multiple threads is not supported, whether the calls run concurrently or are run one after another on separate threads (each thread started and joined before the next is started).
